        ...
        for x, y in queue:
            for i in range(4):
                ...
                if is_in_bounds(next_x, next_y, cols, rows):
                    if not visited[next_y][next_x] and level_map[next_y][next_x] == level_map[y][x] + 1:
...
if __name__ == '__main__':
    ...
    result = 0
    # 2.) perform bfs on each tile with value 0 and count reachable tiles with number 9
